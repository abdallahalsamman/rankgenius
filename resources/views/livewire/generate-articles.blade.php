    ...
    <x-tabs selected="simple-mode-tab">
        <x-tab name="simple-mode-tab" label="Simple mode">
            <x-form wire:submit="simpleMode">
                ...
                <x-instruction-step number-class="py-[4px]" number="2" class="mt-5" instruction="Internal/External Linking" />
                <x-input wire:model="sitemap_url" placeholder="Sitemap URL" suffix="Optional" />
                <x-checkbox wire:model="external_linking" label="External Linking" />

                <x-instruction-step number-class="py-[4px]" number="4" class="mt-5" instruction="Article Language" />
                ...
                <div class="grid grid-cols-[1fr_200px] gap-5">
                    ...
                    <x-button label="Generate" type="submit" class="bg-neutral-900 font-semibold hover:bg-gray-700 text-white text-base" spinner="save2" />
                </div>
        ...
        <x-tab name="titles-mode-tab" label="Titles mode">
            <div>
                <x-form wire:submit="titleMode">
                    <x-instruction-step number-class="py-[4px]" number="1" class="mt-5" instruction="Write your titles (1 per line)" />
                    <x-textarea wire:model="titles" placeholder="How to make bread at home?
Best recipes for making bread
What are the different types of bread?" rows="7" class="resize-none text-[16px] leading-6" />
                    <x-instruction-step number-class="py-[4px]" number="2" class="mt-5" instruction="Generate Articles" />
                    <div class="grid grid-cols-[1fr_100px] gap-5">
                        <x-select class="text-base" option_value="name" :options="$languages" wire:model="language" />
                        <x-button label="Generate" type="submit" class="bg-neutral-900 font-semibold text-white hover:bg-gray-700 text-base" spinner="save2" />
                    </div>
                </x-form>
            </div>
        </x-tab>
