<div>
    <div class="flex justify-between mt-10">
        ...
        <div>
            <x-button class="font-semibold text-white btn-sm bg-neutral-900 hover:bg-gray-700" icon="o-plus-small" label="Generate More" link="{{ route('dashboard') }}" />
        </div>
    </div>
    <div class="grid grid-row-4">
        <div class="flex w-full items-center border-b-[1px] border-base-200 px-6 py-3">
            ...
            <div class="w-1/2">
                ...
                <div class="flex items-center">
                    <div class="{{ $statusColor[$batch->status] }} w-fit rounded px-2 py-1 text-sm font-medium" @if ($batch->status === \App\Enums\BatchStatusEnum::IN_PROGRESS->value)
                        wire:poll.5s
                        @endif
                        >
        ...
        <div class="flex w-full items-center border-b-[1px] border-base-200 px-6 py-3">
            ...
            <div class="flex justify-between items-center w-1/2">
                <div @if ($batch->status === \App\Enums\BatchStatusEnum::IN_PROGRESS->value) wire:poll.5s @endif
                    >
    ...
    <div class="flex justify-between my-10">
        <div class="flex gap-2">
            ...
            <a href="{{ route('integrations.index') }}" class="font-semibold text-white btn-sm bg-neutral-900 hover:bg-gray-700 disabled:text-white">Create Integration</a>
            @endif
        </div>
    </div>
    ...
    <div class="flex relative flex-col h-full">
        <div class="bg-white">
            <div class="flex gap-5 items-center px-6 py-4" wire:loading.class="opacity-50">
                <x-button :disabled="$selectedArticleIdx === 0" class="bg-white btn-sm text-neutral-900 hover:border-black hover:bg-neutral-900 hover:text-white" icon="o-arrow-small-left" label="Previous" spinner wire:click="previous" />

                <div class="w-full">
                    <x-select :options="$batch->articles->toArray()" class="pr-8 btn-sm leading-[100%]" option_label="title" wire:model.change="selectedArticleId" />
                </div>

                <x-button :disabled="$selectedArticleIdx ==
                    $batch->articles->count() - 1" class="bg-white btn-sm text-neutral-900 hover:border-black hover:bg-neutral-900 hover:text-white" icon-right="o-arrow-small-right" label="Next" spinner wire:click="next" />
